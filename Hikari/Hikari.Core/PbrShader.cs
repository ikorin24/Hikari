...
public sealed partial class PbrShader : ITypedShader
{
    ...
    private static ReadOnlySpan<byte> Source => """
        ...
        @group(2) @binding(0) var<uniform> c: CameraMatrix;
    ...
    private static readonly Lazy<ImmutableArray<byte>> _shadowShader = new(() =>
    {
        ...
        sb.AppendUtf8("""
            ...
            @vertex fn vs_main(
                ...
                let p: vec4<f32> = lightMatrices[cascade] * u.model * vec4<f32>(pos, 1.0);
            ...
            [
                new()
                {
                    ...
                    OnRenderPass = static (in RenderPassState state) =>
                    {
                        ...
                        state.RenderPass.DrawIndexed(state.Submesh.IndexOffset, state.Submesh.IndexCount, state.Submesh.VertexOffset, 0, DirectionalLight.CascadeCountConst);
                    },
                },
                new()
                {
                    ...
                    OnRenderPass = static (in RenderPassState state) =>
                    {
                        ...
                        state.Material.SetBindGroupsTo(state.RenderPass, state.PassIndex, state.Renderer);
                        state.RenderPass.SetVertexBuffer(0, state.Mesh.VertexBuffer);
                        Debug.Assert(state.Mesh.TangentBuffer.HasValue);
                        state.RenderPass.SetVertexBuffer(1, state.Mesh.TangentBuffer.Value);
                        state.RenderPass.SetIndexBuffer(state.Mesh.IndexBuffer, state.Mesh.IndexFormat);
            ...
            BindGroupLayouts = [
                ...
                BindGroupLayout.Create(screen, new()
                {
                    ...
                    [
                        ...
                        BindGroupLayoutEntry.Sampler(3, ShaderStages.Fragment, SamplerBindingType.Filtering),
                        ...
                        BindGroupLayoutEntry.Sampler(5, ShaderStages.Fragment, SamplerBindingType.Filtering),
                    ],
                }).AddTo(disposable),
