...
""");
        ...
        foreach(var defList in data) {
            ...
            foreach(var def in defList.Defs) {
                ...
                sb.AppendLine($$"""
        if(typeof(TFrom) == typeof({{tFrom}}) && typeof(TTo) == typeof({{tTo}})) {
            return TryMapPrivate(Unsafe.As<TFrom, {{tFrom}}>(ref source), out Unsafe.As<TTo, {{tTo}}>(ref to));
        }
""");
            }
        }
...
""");
    ...
    private static bool TryMapPrivate({{tFrom}} source, out {{tTo}} to)
    {
        switch(source) {
""");
                foreach(var (vFrom, vTo) in def.Values) {
                    sb.AppendLine($$"""
            case {{tFrom}}.{{vFrom}}: { to = (({{tTo}}){{vTo}}); return true; }
""");
                }
                sb.AppendLine($$"""
